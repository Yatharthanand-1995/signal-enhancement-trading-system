...
from src.models.ml_ensemble import LSTMXGBoostEnsemble  # 🚀 NEW: ML Integration
...
class IntegratedSignal:
    ...
    ml_contribution: float  # 🚀 NEW: ML ensemble contribution
...
class EnhancedSignalIntegrator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 🚀 NEW: Initialize ML ensemble
        self.ml_ensemble = LSTMXGBoostEnsemble()
        self.ml_ensemble_initialized = False
    ...
    def generate_integrated_signal(
        ...
        try:
            ...
            integrated_signal = IntegratedSignal(
                ...
                ml_contribution=weighted_signals.get('ml_ensemble', 0.0),  # 🚀 NEW: ML contribution
    ...
    def _generate_component_signals(
        ...
        try:
            ...
            # 🚀 NEW: Add ML Ensemble Predictions
            ml_signals = self._generate_ml_signals(symbol, market_data)
            signals.update(ml_signals)
            
        except Exception as e:
            logger.warning(f"Error generating component signals for {symbol}: {e}")
            # Return minimal signals including ML defaults
            signals = {
                'technical': 0.5, 
                'volume': 0.5, 
                'momentum': 0.5, 
    ...
    def _generate_ml_signals(self, symbol: str, market_data: pd.DataFrame) -> Dict[str, float]:
        """🚀 NEW: Generate ML-based signal predictions"""
        ml_signals = {}
        
        try:
            if len(market_data) < 60:  # Need sufficient data for ML
                logger.debug(f"Insufficient data for ML predictions ({len(market_data)} days)")
                return {'ml_ensemble': 0.5, 'ml_confidence': 0.5, 'ml_lstm': 0.5, 'ml_xgb': 0.5}
            
            # Initialize ML ensemble if not done
            if not self.ml_ensemble_initialized:
                logger.info("Initializing ML ensemble for live predictions...")
                try:
                    # Try to load pre-trained models
                    self.ml_ensemble.load_models('models/')
                    self.ml_ensemble_initialized = True
                    logger.info("Pre-trained ML models loaded successfully")
                except:
                    logger.warning("No pre-trained models found, ML ensemble initialized with defaults")
                    self.ml_ensemble_initialized = True
            
            # Get ML predictions
            prediction, confidence, explanation = self.ml_ensemble.predict_single(market_data)
            
            # Convert ML prediction to signal format
            # prediction is expected return, convert to 0-1 signal strength
            if prediction > 0:  # Positive prediction
                ml_signal_strength = 0.5 + min(prediction * 10, 0.5)  # 0.5 to 1.0
            else:  # Negative prediction  
                ml_signal_strength = 0.5 - min(abs(prediction) * 10, 0.5)  # 0.0 to 0.5
            
            ml_signals['ml_ensemble'] = ml_signal_strength
            ml_signals['ml_confidence'] = confidence
            
            # Individual model signals if available
            if 'lstm_prediction' in explanation:
                lstm_pred = explanation['lstm_prediction']
                ml_signals['ml_lstm'] = 0.5 + np.tanh(lstm_pred * 5) * 0.5
                
            if 'xgb_prediction' in explanation:
                xgb_pred = explanation['xgb_prediction']
                ml_signals['ml_xgb'] = 0.5 + np.tanh(xgb_pred * 5) * 0.5
            
            logger.debug(f"ML signals generated for {symbol}: ensemble={ml_signal_strength:.3f}, confidence={confidence:.3f}")
            
        except Exception as e:
            logger.error(f"Error generating ML signals for {symbol}: {e}")
            # Fallback to neutral signals
            ml_signals = {'ml_ensemble': 0.5, 'ml_confidence': 0.5, 'ml_lstm': 0.5, 'ml_xgb': 0.5}
    ...
    def _apply_regime_weighting(
        ...
        # 🚀 UPDATED: Base weights including ML components
        base_weights = {
            'technical': 0.20,        # Reduced from 0.30
            'volume': 0.20,           # Reduced from 0.25  
            'momentum': 0.15,         # Reduced from 0.25
            'mean_reversion': 0.15,   # Reduced from 0.20
            'ml_ensemble': 0.25,      # NEW: Primary ML signal
            'ml_confidence': 0.05     # NEW: ML confidence weighting
        }
    ...
    def _get_regime_weight_adjustments(self, regime_info: RegimeInfo) -> Dict[str, float]:
        ...
        if regime_info.volatility_level == 'High':
            ...
            adjustments['ml_ensemble'] = 1.1   # 🚀 ML slightly more reliable in volatility
            adjustments['ml_confidence'] = 0.9  # Lower confidence weighting in volatility
            ...
            adjustments['ml_ensemble'] = 1.2   # 🚀 ML performs better in stable conditions
            adjustments['ml_confidence'] = 1.1  # Higher confidence weighting when stable
        ...
        if regime_info.trend_direction == 'Bull':
            ...
            adjustments['ml_ensemble'] = adjustments.get('ml_ensemble', 1.0) * 1.1  # 🚀 ML boost in bull markets
            ...
            adjustments['ml_ensemble'] = adjustments.get('ml_ensemble', 1.0) * 1.0  # 🚀 ML neutral in bear markets
        
        # Default to 1.0 for any missing adjustments
        for signal_type in ['technical', 'volume', 'momentum', 'mean_reversion', 'ml_ensemble', 'ml_confidence']:
